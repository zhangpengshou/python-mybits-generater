...
def get_jdbc_type_from_sql_type(column_type):
    ...
    if column_type.find("int4") > -1:
        ...
        jdbc_type = "TIMESTAMP"
    elif column_type.find("numeric") > -1:
        jdbc_type = "NUMERIC"
...
def get_java_type_from_sql_type(column_type, return_fullname=False):
    ...
    if column_type.find("int4") > -1:
        ...
        if return_fullname:
            mapping_list[0] = "java.math.BigDecimal"
        else:
            mapping_list[0] = "BigDecimal"
        mapping_list[1] += "import java.math.BigDecimal;\r"
    elif column_type.find("json") > -1:
        if return_fullname:
...
def generate_single_interface(schema_name, table_name, is_view=False):
    ...
    interface_header += "\rpublic interface I{0}Repository {1}\r".format(second_word_behind_capitalize(table_name, "_", True), "{")
    ...
    file_interface = open(os.path.join(interface_path, "I{0}Repository.java".format(second_word_behind_capitalize(table_name, "_", True))), mode="w", encoding="UTF-8")
    file_interface.write(interface_header + interface_body)
    file_interface.close()


# 根据schema和表名生成单个服务repositorys
def generate_single_repository(schema_name, table_name, is_view=False):
    repository_header = repository_body = ""
    all_columns = get_all_columns(schema_name, table_name)
    primary_key_column = get_primary_key_column_from_clumns_list(table_name, all_columns)

    repository_header += "package {0};\r".format(global_repositorys_name_space)

    repository_header += "\rimport java.util.List;\r"
    repository_header += "import javax.annotation.Resource;\r"
    repository_header += "import org.springframework.stereotype.Repository;\r"
    repository_header += "import {0}.I{1}Repository;\r".format(global_interfaces_name_space, second_word_behind_capitalize(table_name, "_", True))
    repository_header += "import {0}.{1}Mapper;\r".format(global_java_mapper_name_space, second_word_behind_capitalize(table_name, "_", True))
    repository_header += "import {0}.{1};\r".format(global_model_name_space, second_word_behind_capitalize(table_name, "_", True))

    repository_body += "\r@Repository\r"
    repository_body += '''public class {0}Repository implements I{0}Repository {1}\r'''.format(second_word_behind_capitalize(table_name, "_", True), "{")
    repository_body += "    @Resource\r"
    repository_body += "    private {0}Mapper {1}Mapper;\r\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"))

    if is_view == False:
        # add
        repository_body += "    @Override\r"
        repository_body += "    public Integer add({0} {1}){2}\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "        if({0} == null){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "            return 0;\r"
        repository_body += "        }\r"
        repository_body += "        else{ \r"
        repository_body += "            return {0}Mapper.insert({0});\r".format(second_word_behind_capitalize(table_name, "_"))
        repository_body += "        }\r"
        repository_body += "    }\r"

    repository_body += "    @Override\r"
    repository_body += "    public Integer count({0} {1}){2}\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"), "{")
    repository_body += "        if({0} == null){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
    repository_body += "            return 0;\r"
    repository_body += "        }\r"
    repository_body += "        else{ \r"
    repository_body += "            return {0}Mapper.count({0});\r".format(second_word_behind_capitalize(table_name, "_"))
    repository_body += "        }\r"
    repository_body += "    }\r"
    ...
    if is_view == False:
        ...
        if primary_key_parmas_group != "":
            repository_body += "    @Override\r"
            repository_body += "    public Integer delete({0} {1}){2}\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"), "{")
            repository_body += "        if({0} == null){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
            repository_body += "            return 0;\r"
            repository_body += "        }\r"
            repository_body += "        else{ \r"
            repository_body += "            return {0}Mapper.delete({0});\r".format(second_word_behind_capitalize(table_name, "_"))
            repository_body += "        }\r"
            repository_body += "    }\r"

        # update
        repository_body += "    @Override\r"
        repository_body += "    public Integer update({0} {1}){2}\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "        if({0} == null){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "            return 0;\r"
        repository_body += "        }\r"
        repository_body += "        else{ \r"
        repository_body += "            return {0}Mapper.updateByPrimaryKey({0});\r".format(second_word_behind_capitalize(table_name, "_"))
        repository_body += "        }\r"
        repository_body += "    }\r"

        # add or update
        repository_body += "    @Override\r"
        repository_body += "    public Integer addOrUpdate({0} {1}){2}\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "        if({0} == null){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "            return 0;\r"
        repository_body += "        }\r"
        repository_body += "        else{ \r"
        repository_body += "            if({0}Mapper.count({0}) == 0){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "                return {0}Mapper.insert({0});\r".format(second_word_behind_capitalize(table_name, "_"))
        repository_body += "            }\r"
        repository_body += "            else{\r"
        repository_body += "                return {0}Mapper.updateByPrimaryKey({0});\r".format(second_word_behind_capitalize(table_name, "_"))
        ...
        if primary_key_parmas_group != "":
            repository_body += "    @Override\r"
            repository_body += "    public {0} getFirst({1}) {2}\r".format(second_word_behind_capitalize(table_name, "_", True), primary_key_parmas_group, "{")

            repository_body += "        if({0}){1}\r".format(primary_key_parmas_value_not_null_group, "{")
            repository_body += "            return null; \r"
            repository_body += "        }\r"
            repository_body += "        else{\r"
            repository_body += "            {0} {1} = new {0}();\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"))
            repository_body += primary_key_set_value_group
            repository_body += "            return {0}Mapper.first({0});\r".format(second_word_behind_capitalize(table_name, "_"))
            repository_body += "        }\r"
            repository_body += "    }\r"
        ...
        # get First
        repository_body += "    @Override\r"
        repository_body += "    public {0} getFirst({0} {1}){2}\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "        if({0} == null){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
        repository_body += "            return null;\r"
        repository_body += "        }\r"
        repository_body += "        else{ \r"
        repository_body += "            return {0}Mapper.first({0});\r".format(
    ...
    # get List
    repository_body += "    @Override\r"
    repository_body += "    public List<{0}> getList({0} {1}){2}\r".format(second_word_behind_capitalize(table_name, "_", True), second_word_behind_capitalize(table_name, "_"), "{")
    repository_body += "        if({0} == null){1}\r".format(second_word_behind_capitalize(table_name, "_"), "{")
    repository_body += "            return null;\r"
    repository_body += "        }\r"
    repository_body += "        else{ \r"
    repository_body += "            return {0}Mapper.select({0});\r".format(second_word_behind_capitalize(table_name, "_"))
    repository_body += "        }\r"
    repository_body += "    }\r"

    repository_body += "}"

    repository_path = os.path.join(generate_path, "api/repositorys")
    if (os.path.exists(repository_path) == False):
        os.makedirs(repository_path)
    file_service = open(os.path.join(repository_path, "{0}Repository.java".format(second_word_behind_capitalize(table_name, "_", True))), mode="w", encoding="UTF-8")
    file_service.write(repository_header + repository_body)
    file_service.close()
...
def generate_single_controller(schema_name, table_name, is_view=False):
    ...
    if is_view == False:
        ...
        if len(primary_key_column) >= 1:
            ...
            for column in primary_key_column:
                if column_index == 0:
                    primary_keys_added += "{0}Repository.add({0})".format(second_word_behind_capitalize(table_name, "_"))
                else:
                    primary_keys_added += " && {0}Repository.add({0})".format(second_word_behind_capitalize(table_name, "_"))
                column_index += 1
